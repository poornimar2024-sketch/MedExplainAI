...
def create_pdf_report(
    ...
    if heatmap_img is not None:
        content.append(Paragraph("Grad-CAM Visualization", styles["Heading2"]))
        ...
        content.append(RLImage(img_buffer, width=320, height=320))
        content.append(Spacer(1, 10))

        content.append(Paragraph(
            "Heatmap Colour Interpretation:", styles["Heading3"]
        ))
        content.append(Paragraph(
            "Red / Yellow regions indicate areas where the model strongly focused while making the prediction.",
            styles["Normal"]
        ))
        content.append(Paragraph(
            "Green regions indicate areas of moderate importance.",
            styles["Normal"]
        ))
        content.append(Paragraph(
            "Blue regions indicate areas of low importance or regions largely ignored by the model.",
            styles["Normal"]
        ))

        content.append(Spacer(1, 8))

        if abnormality_description:
            content.append(Paragraph("AI Description of Abnormality Pattern:", styles["Heading3"]))
            content.append(Paragraph(abnormality_description, styles["Normal"]))
    ...
    with c1:
        ...
        <div class="glass-card">
            ...
            <ul>
                ...
                <li>Downloadable PDF report with heatmap</li>
            </ul>
    ...
    if show_gradcam:
        ...
        try:
            ...
            if grads is None:
                ...
                st.markdown("### 🔍 Heatmap Colour Interpretation")
                st.write("""
- 🔴 **Red / Yellow regions** → Areas the model strongly focused on  
- 🟢 **Green regions** → Areas of moderate importance  
- 🔵 **Blue regions** → Areas of low importance  

These highlighted regions indicate where the AI detected patterns associated with pneumonia.
                """)
    ...
    if patient_name and patient_age:
        pdf_data = create_pdf_report(
            ...
            result_label,
            report_items,
            heatmap_img=superimposed_img if show_gradcam and superimposed_img is not None else None,
    ...
    # ---------------- MODEL EXPLANATION + DETAILS + LIMITATIONS ----------------
